...
const StoryCard: React.FC<StoryCardProps> = ({ story, onClick }) => {
  return (
    ...
    >
      ...
      <div className="absolute top-1 left-1 md:top-2 md:left-2 text-xs opacity-40 md:opacity-50">✨</div>
      <div className="absolute top-2 right-6 md:top-3 md:right-8 text-xs opacity-30 md:opacity-40">🌟</div>
      ...
      <div className="relative h-32 md:h-44 overflow-hidden bg-gradient-to-br from-peru-blue/10 to-peru-purple/10 flex items-center justify-center">
        ...
        <div className="text-6xl md:text-8xl group-hover:scale-110 transition-transform duration-500">
          {story.coverImage}
        </div>
      ...
      <CardContent className="p-3 md:p-5 relative">
        ...
        <div className="hidden md:block absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-all duration-500 transform group-hover:rotate-12">
          <div className="bg-peru-yellow text-peru-blue px-2 py-1 rounded-full text-xs font-comic font-bold">
            ¡Toca para leer!
          </div>
        </div>
      </CardContent>
